...
const manifest = {
  ...
  version: "2.0.0",
  name: "🎬 ArabP2P Movies & Series",
  description: "أفلام ومسلسلات مع دعم حلقات التورنت",
  resources: ["catalog", "meta", "stream"],
  types: ["movie", "series"],
  catalogs: [
    { type: "movie", id: "arabp2p_movies", name: "أفلام ArabP2P", extra: [{ name: "search", isRequired: false }] },
    { type: "series", id: "arabp2p_series", name: "مسلسلات ArabP2P", extra: [{ name: "search", isRequired: false }] }
  ],
...
builder.defineStreamHandler(async ({ type, id }) => {
  ...
  if (type === 'series') {
    ...
    const video = item.videos.find(v => String(v.season) === String(season) && String(v.episode) === String(episode));
